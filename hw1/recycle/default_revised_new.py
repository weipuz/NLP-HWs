...
optparser.add_option("-i", "--inputfile", dest="input", default=os.path.join('data', 'input_seg1_423'), help="input file to segment")
...
class Pdist(dict):
    ...
    def __call__(self, key): 
        if key in self: 
            ...
            return self.missingfn(key, 2*self.N)#I change it to 1/2N here, not really necessary.
...
def word_seg(input_line):
    ...
    for i in range(word_max_len):
        ...
        not_found = log10(Pw.missingfn(current_word,2*Pw.N))#log_prob, notice minus value
    ...
    if len(myheap) ==0 : 
        heappush(myheap, make_entry(input_line[0:1].encode('utf-8'),0, not_found, None)) #the log_prob = not_found here, you can change it.
    ...
    while myheap:        
        ...
        for i in range(word_max_len):
            if end_index+i <= line_len-1:
            	...
            	not_found  = log10(Pw.missingfn(current_word,2*Pw.N))
        ...
        #print insert_flag
        if end_index != line_len-1:#if no matching word
            if insert_flag == 0:
                myheap.append(make_entry(input_line[end_index + 1: end_index + 2].encode('utf-8'),end_index+1,not_found,end_index))
